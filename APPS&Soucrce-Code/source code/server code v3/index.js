...
const parentClients = new Map() // Store parent connections
const childDevices = new Map() // Store child device connections
const parentChildLinks = new Map() // Map parent codes to child devices
    ...
    res.send(`
        <div style="text-align: center; font-family: Arial, sans-serif; padding: 50px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; min-height: 100vh;">
            ...
            <div style="margin: 30px 0;">
                <a href="/parent-dashboard" style="background: #1976D2; color: white; padding: 15px 30px; text-decoration: none; border-radius: 25px; font-size: 18px; margin: 10px;">
                    👨‍👩‍👧 Parent Dashboard
                </a>
                <br><br>
            ...
            <p style="margin-top: 30px; opacity: 0.8;">Real-time Android device monitoring & child safety system</p>
        </div>
    `)
})
...
app.get('/parent-dashboard', function (req, res) {
    res.sendFile(__dirname + '/parent-dashboard.html')
})
...
app.post('/api/child-safety/register-parent', function (req, res) {
    const { parentId, linkingCode } = req.body
    
    if (!parentId || !linkingCode) {
        return res.json({ success: false, message: 'Invalid data' })
    }
    
    parentClients.set(parentId, {
        linkingCode: linkingCode,
        children: [],
        connected: true
    })
    
    res.json({ success: true, parentId: parentId })
})

app.post('/api/child-safety/register-child', function (req, res) {
    const { deviceId, childName, parentCode, deviceInfo } = req.body
    
    if (!deviceId || !childName || !parentCode) {
        return res.json({ success: false, message: 'Invalid data' })
    }
    
    let parentId = null
    parentClients.forEach((parent, pId) => {
        if (parent.linkingCode === parentCode) {
            parentId = pId
        }
    })
    
    if (!parentId) {
        return res.json({ success: false, message: 'Invalid parent code' })
    }
    ...
    childDevices.set(deviceId, childData)
    const parent = parentClients.get(parentId)
    parent.children.push(deviceId)
    
    res.json({ success: true, message: 'Child device linked successfully' })
})

app.get('/api/child-safety/children/:parentId', function (req, res) {
    ...
    const children = parent.children.map(childId => {
        const child = childDevices.get(childId)
        return child || null
    }).filter(c => c !== null)
...
app.post('/api/child-safety/command', function (req, res) {
    ...
    appSocket.clients.forEach(function each(ws) {
        if (ws.uuid == deviceId && ws.isChildDevice) {
...
app.get('/api/child-safety/location/:deviceId', function (req, res) {
    const deviceId = req.params.deviceId
    const child = childDevices.get(deviceId)
    ...
    res.json({ 
        ...
        timestamp: child.lastUpdate
    })
})

app.get('/api/child-safety/app-usage/:deviceId', function (req, res) {
    const deviceId = req.params.deviceId
    const child = childDevices.get(deviceId)
...
appSocket.on('connection', (ws, req) => {
    ...
    const deviceType = req.headers.devicetype || 'normal' // 'child', 'parent', 'normal'

    ws.uuid = uuid
    ws.isChildDevice = (deviceType === 'child')
    ws.isParentClient = (deviceType === 'parent')
    ...
    ws.on('message', function(message) {
        try {
            ...
            // Handle child device messages
            if (ws.isChildDevice) {
                if (data.type === 'child_device_register') {
                    const childInfo = childDevices.get(ws.uuid)
                    if (childInfo) {
                        // Notify parent about connection
                        appSocket.clients.forEach(function each(client) {
                            if (client.isParentClient && client.parentId === childInfo.parentId) {
                                client.send(JSON.stringify({
                                    type: 'child_device_connected',
                                    deviceId: ws.uuid,
                    ...
                    // Store and forward monitoring data
                    const childInfo = childDevices.get(ws.uuid)
                    if (childInfo) {
                        childInfo.location = data.location
                        childInfo.appUsage = data.appUsage
                        childInfo.screenTime = data.screenTime
                        childInfo.lastUpdate = Date.now()
                        
                        // Forward to parent
                        appSocket.clients.forEach(function each(client) {
                            if (client.isParentClient && client.parentId === childInfo.parentId) {
                                client.send(JSON.stringify({
                                    type: 'child_device_update',
                                    deviceId: ws.uuid,
                                    ...data
                                }))
                            }
            ...
            // Handle parent client messages
            if (ws.isParentClient) {
                if (data.type === 'parent_register') {
                    ws.parentId = data.parentId
                } else if (data.type === 'parent_command') {
                    // Forward command to child device
                    appSocket.clients.forEach(function each(client) {
                        if (client.uuid === data.deviceId && client.isChildDevice) {
            ...
            // Regular message handling for non-JSON messages
        }
    })
    
    ws.on('close', function () {
        // Handle child device disconnection
        if (ws.isChildDevice) {
            const childInfo = childDevices.get(ws.uuid)
            if (childInfo) {
                childInfo.isOnline = false
                
                // Notify parent
                appSocket.clients.forEach(function each(client) {
                    if (client.isParentClient && client.parentId === childInfo.parentId) {
                        client.send(JSON.stringify({
                            type: 'child_device_disconnected',
                            deviceId: ws.uuid
                        }))
                    }
                })
